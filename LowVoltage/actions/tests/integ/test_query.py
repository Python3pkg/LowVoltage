...
class QueryLocalIntegTests(_tst.LocalIntegTestsWithTableHR):
    ...
    def test_complex_query(self):
        r = self.connection(
            _lv.Query("Aaa").key_eq("h", "0").key_between("r", 42, 44)
...
class QueryConnectedIntegTests(_tst.ConnectedIntegTestsWithTable):
    ...
    def test_return_consumed_capacity_total(self):
        r = self.connection(_lv.Query(self.table).key_eq("tab_h", "0").return_consumed_capacity_total())
